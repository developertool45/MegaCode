...
const registerUser = asyncHandler(async (req, res) => {
  ...
  await user.save();
  return res.send(new ApiResponse(201, {
...
const loginUser = asyncHandler(async (req, res) => {
  ...
  try {
    ...
    const loggedInUser = await User.findById(user._id).select("-password -refreshToken -refreshTokenExpiry"); 
...
const resendVerificationEmail = asyncHandler(async (req, res) => {
  ...
  try {
    ...
    const user = await User.findOne({ email }).select("-password -refreshToken -refreshTokenExpiry -emailVerificationToken -emailVerificationTokenExpiry");
